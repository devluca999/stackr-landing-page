...
export default function Hero() {
  ...
  return (
    <section style={{ minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"130px 24px 60px", position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ position:"absolute", top:"15%", left:"50%", transform:"translateX(-50%)", width:700, height:500, background:"radial-gradient(ellipse, rgba(255,61,0,0.055) 0%, transparent 65%)", pointerEvents:"none" }}/>

      {/* Molecular SVG decal top-right */}
      <svg width="160" height="160" viewBox="0 0 160 160" style={{ position:"absolute", top:110, right:"7%", opacity:0.09, pointerEvents:"none" }}>
      ...
      <svg width="110" height="130" viewBox="0 0 110 130" style={{ position:"absolute", bottom:"18%", left:"5%", opacity:0.07, pointerEvents:"none" }}>
      ...
      {/* Pill badge */}
      <div className="animate-fade-in" style={{ display:"inline-flex", alignItems:"center", gap:8, padding:"5px 16px 5px 7px", borderRadius:30, background:"var(--surface)", border:"1px solid var(--border-warm)", marginBottom:32 }}>
      ...
      <div className="animate-slide-up d6" style={{ display:"flex", gap:12, flexWrap:"wrap", justifyContent:"center", marginBottom:72 }}>
        {[
          ...
          <div key={store} style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 20px", borderRadius:12, background:"var(--surface)", border:"1px solid var(--border)", cursor:"pointer", transition:"border-color 0.2s" }}
            onMouseEnter={e=>(e.currentTarget.style.borderColor="rgba(255,106,0,0.25)")}
      ...
      <div style={{ width:"100vw", overflow:"hidden", position:"relative", marginLeft:"-50vw", left:"50%" }}>
        <div style={{ display:"flex", gap:14, width:"max-content" }} className="animate-carousel">
          {CAROUSEL.map((s,i) => (
            <div key={i} style={{ minWidth:230, padding:"16px 18px", background:"var(--surface)", border:"1px solid var(--border)", borderRadius:14, flexShrink:0 }}>
