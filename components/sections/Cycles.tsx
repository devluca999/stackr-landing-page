...
export default function Cycles() {
  return (
    <section style={{padding:"120px 48px",background:"var(--surface)",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{maxWidth:1100,margin:"0 auto",position:"relative"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:24}}>
          {CYCLES.map((c,i)=>(
            <div key={i} className="card-base" style={{padding:"28px 28px"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:10,marginBottom:20}}>
                {c.items.map((item,j)=>(
                  <div key={j} style={{padding:"12px 14px",borderRadius:12,background:"var(--surface-2)",border:"1px solid var(--border)",display:"flex",flexDirection:"column",gap:4}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      <span style={{fontFamily:"Syne,sans-serif",fontWeight:700,fontSize:13,color:"var(--text)"}}>{item.compound}</span>
                      <span style={{fontFamily:"DM Sans,sans-serif",fontSize:11,fontWeight:500,color:c.color}}>{item.dose}</span>
                    </div>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      <span style={{fontFamily:"DM Sans,sans-serif",fontSize:11,color:"var(--text-muted)"}}>{item.timing}</span>
                    </div>
                    <p style={{fontFamily:"DM Sans,sans-serif",fontSize:11,color:"var(--text-muted)",lineHeight:1.4}}>{item.notes}</p>
                  </div>
                ))}
              </div>
              <div style={{padding:"14px 16px",borderRadius:10,background:"var(--bg)",border:"1px solid var(--border)"}}>
                <p style={{fontFamily:"DM Sans,sans-serif",fontSize:13,color:"var(--text-secondary)",lineHeight:1.65}}>
                  <strong style={{color:"var(--flame-3)",fontWeight:600}}>Protocol: </strong>{c.protocol}
                </p>
              </div>
            </div>
          ))}
